...
.. code-block:: shell
        ...
        $ docker build --no-cache -t bdc-stac:0.7.0-0 .


The above command will create a Docker image named `bdc-stac` and tag `0.7.0-0`, as one can see with the `docker images` command:
...
.. code-block:: shell
        ...
        bdc-stac                                            0.7.0-0             44651ac917e4        16 hours ago        333MB
...
The `docker run` command can be used to launch a container from the image `bdc-stac:0.7.0-0`. The command below shows an example on how to accomplish the launch of a container:
