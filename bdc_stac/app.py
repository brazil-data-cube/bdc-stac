...
def collection_items(collection_id):
    ...
    gjson = make_geojson(items, links, page=int(request.args.get('page', 1)),
...
def stac_search():
    ...
    items = get_collection_items(collections=collections, bbox=bbox, time=time, ids=ids, type=type, bands=bands)
    ...
    gjson = make_geojson(items, links=links, page=page, limit=limit)
